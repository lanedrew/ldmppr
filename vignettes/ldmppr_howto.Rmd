...
To illustrate the use of the `ldmppr` package, we will be using the included `small_example_data` dataset. This dataset consists of locations and sizes for 88 points in a 25x25 square domain.
...
Details for generating and obtaining these datasets can be found in the `data-raw` directory of the package repository.
...
In practice, we recommend using a denser grid for the optimization process to ensure that the global optimum is found. We also note that the function `estimate_parameters_sc_parallel()` can be used to fit the model for varying values of `delta` in parallel to identify the optimal mapping between sizes and arrival times.
